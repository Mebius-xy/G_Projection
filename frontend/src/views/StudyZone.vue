<template>
  <div class="font-sans h-full flex flex-col">
    ...
    <div class="flex gap-6 flex-1 overflow-hidden">
      ...
      <!-- 左侧分类导航 -->
      <div class="w-48 bg-white rounded-xl shadow-sm border border-gray-100 p-3 flex-shrink-0 flex flex-col h-full">
      ...
      <div class="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 p-6 overflow-y-auto relative">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
          <h2 class="text-xl font-bold text-gray-800">{{ currentNavName }}资源</h2>
          <div class="flex gap-2">
            <select class="border border-gray-200 rounded-md text-sm px-3 py-1.5 focus:outline-none focus:border-blue-500">
              <option>最多阅读</option>
              <option>最新发布</option>
            </select>
          </div>
        ...
        <div v-if="activeNav !== 'art'">
          ...
          <div v-else class="space-y-4">
            <div v-for="q in realQuestions" :key="q.id" class="p-5 border border-gray-100 rounded-xl hover:shadow-md transition cursor-pointer group">
              <div class="flex justify-between items-start">
                <div>
                  <h3 class="text-lg font-bold text-gray-800 group-hover:text-blue-600 transition">{{ q.title }}</h3>
                  <p class="text-sm text-gray-500 mt-2 line-clamp-2">{{ q.description }}</p>
                  <div class="flex gap-3 mt-4 text-xs">
                    <span class="bg-gray-100 text-gray-600 px-2 py-1 rounded font-mono">{{ q.tag }}</span>
                    <span class="text-gray-400 flex items-center"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>{{ q.views }} 次阅读</span>
                  </div>
                </div>
                <!-- 🌟 修复点：去掉了重复的 :class，改用一个优雅的小工具函数 🌟 -->
                <span class="px-2 py-1 rounded text-xs font-bold whitespace-nowrap border" :class="getDifficultyClass(q.difficulty)">
                  {{ q.difficulty }}
                </span>
              </div>
            </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          <div v-for="art in mockArts" :key="art.id" class="border border-gray-100 rounded-xl overflow-hidden hover:shadow-lg transition cursor-pointer group">
            <div class="h-40 bg-gray-200 overflow-hidden relative">
              <!-- 🌟 修复点：加上了 alt 属性消除黄线警告 🌟 -->
              <img :src="art.img" alt="艺术作品封面" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
...
const navs = [
  { id: 'algo', name: '算法真题 (爬虫抓取)' },
  { id: 'math', name: '数学建模论文库' },
  { id: 'dev', name: '开发实战库' },
...
// 🌟 新增小工具：处理难度徽章的颜色类名，保持模板干净 🌟
const getDifficultyClass = (diff) => {
  const map = {
    '困难': 'text-red-500 bg-red-50 border-red-100',
    '中等': 'text-yellow-600 bg-yellow-50 border-yellow-100',
    '简单': 'text-green-500 bg-green-50 border-green-100'
  }
  // 如果遇到未知的难度，给个默认灰色
